...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      subroutine loadRT_ppc(pbeta,mass,mratio)
            ...
            do l = 1, int(Ni_tot*ratio,4)
                  ...
                  m_arr(l) = mass
                  mrat(l) = mratio
                  beta_p(l) = real(Ni_tot*ratio*procnum)/(vol/2.0)/(np_bottom*((1.0-tanh((xp(l,3)-qz(nz/2))/Lo))/2.0) ) !   Beta =  Ni_tot/volume  /  (density)
                  !write(*,*) 'beta_p...',beta_p(l),vol,np_bottom,ratio
...
!            enddo
            ...
            do l = 1, Ni_tot
                  do m=1,3
                    ...
                    input_E = input_E + 0.5*m_arr(l)*(vp(l,m)*km_to_m)**2/(beta * beta_p(l))
                    input_p(m) = input_p(m) + m_arr(l) * vp(l,m) / (beta * beta_p(l))
                    !beta_p(l) = real(Ni_tot*(1.0-ratio)*procnum)/vol/np_top + real(Ni_tot*ratio*procnum)/vol/(np_bottom*((1.0-tanh((xp(l,3)-qz(nz/2))/Lo))/2.0) ) 
                  enddo
            enddo
