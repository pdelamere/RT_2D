...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      subroutine load_RT(vth,Ni_tot_1,mass,mratio)
            ...
            do i=1,nx
            do j=1,ny
                  grav(i,j,:)=-grav0*(tanh((qz(:)-qz(nz/2) - 180*delz)/(10*delz))-tanh((qz(:)-qz(nz/2)+180*delz)/(10*delz)))
            enddo
            enddo
            ...
            do k=1,nz
                  vbal(k) = sqrt(2*sum(rho(k:nz)*grav(nx/2,2,k:nz)*dz_grid(k:nz))/rho(k))
...
!                  beta_p(l) = beta_particle
                  ...
                  vp(l,3) = (vbal(k))*sqrt(-log(pad_ranf()))*cos(PI*pad_ranf())+0.0*exp(-(xp(l,3)-qz(nz/2))**2/(5*dz_grid(nz/2))**2)*cos(2*PI*(qx(nx-1)-xp(l,1))/(qx(nx-1)-qx(1)))
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      subroutine load_RT_pad(vth,Ni_tot_1,mass,mratio)
            use dimensions
            use mult_proc
            use MPI
            use boundary
            use inputs, only: PI, vsw, dx, dy, km_to_m, beta_particle, kboltz, mion, amp, grad, delz, Lo, nf_init, &
                 np_top, np_bottom, vth_top, vth_bottom
            use grid, only: qx,qy,qz,dz_grid
            use gutsp
            use var_arrays, only: vp,vp1,xp,input_p,Ni_tot,input_E,m_arr,mrat,beta,beta_p,grav,mix_ind,mixed,mix_cnt
            implicit none
...
!            rho = (amp-1)/2*tanh((qz(:)-qz(nz/2))/Lo)+(amp+1)/2
            ...
            vbal(1) = vth_top
            do k = 2,nz 
               vbal(k) = sqrt((vbal(1))**2 - (vth_bottom**2/np_top)*(npb(k)-npb(1)) + &
...
!            enddo
            ...
            do l = 1,Ni_tot
               ...
               vp(l,1) = (vth_bottom)*sqrt(-log(pad_ranf()))*cos(PI*pad_ranf())
               vp(l,2) = (vth_bottom)*sqrt(-log(pad_ranf()))*cos(PI*pad_ranf())
               vp(l,3) = (vth_bottom)*sqrt(-log(pad_ranf()))*cos(PI*pad_ranf())+&
            ...
            Ni_tot_2 = Ni_tot + 1
            Ni_tot_p_t = 2.0*nint(Ni_tot*(np_top/np_bottom))
